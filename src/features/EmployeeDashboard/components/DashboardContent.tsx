...
const DashboardContent = () => {
  ...
  if (opsLoading && !metrics) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center gap-4 text-neutral-500">
        <Loader2 className="animate-spin text-indigo-500 w-10 h-10" />
        <p className="text-[10px] font-black uppercase tracking-[0.4em]">Synchronizing OPS Command...</p>
      </div>
    );
  }

  return (
    <div className="p-6 md:p-10 text-neutral-100 font-sans">
      <div className="max-w-7xl mx-auto">
        <header className="mb-10 flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter text-white leading-none">Operations Command</h1>
            <p className="text-neutral-500 text-[10px] font-bold uppercase tracking-[0.2em] mt-2 border-l-2 border-indigo-500 pl-3">
              Fleet & Workforce Management System
            </p>
          </div>
          <div className="text-right hidden md:block">
            <p className="text-[10px] font-black text-neutral-600 uppercase tracking-widest">System Status</p>
            <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest animate-pulse">● Online</p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-[#111111] border border-neutral-800 rounded-[32px] p-8 shadow-2xl relative overflow-hidden">
            <div className="flex items-center justify-between mb-8 relative z-10">
              <div>
                <h3 className="font-black uppercase text-xs tracking-[0.2em] text-white">Live Trip Monitor</h3>
                <p className="text-[9px] text-neutral-500 uppercase font-bold mt-1">Real-time log of today's movements</p>
              </div>
              <span className="text-[9px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-3 py-1 rounded-full font-black uppercase tracking-tighter">Live Trace</span>
            </div>

            <div className="space-y-3 relative z-10">
              {liveTrips.length > 0 ? liveTrips.map((trip: any, i: number) => (
                <div key={trip._id} className="bg-[#0d0d0f] border border-neutral-800 p-5 rounded-2xl flex items-center justify-between group hover:border-indigo-500/40 transition-all cursor-default">
