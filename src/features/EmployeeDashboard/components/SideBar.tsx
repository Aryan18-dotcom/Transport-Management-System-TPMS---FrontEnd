    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-[#0d0d0f]/80 backdrop-blur-md border-b border-white/5 z-40 px-4 flex items-center">
        ...
        <div className="ml-4 flex items-center gap-2">
          <div className="w-6 h-6 bg-indigo-600 rounded flex items-center justify-center text-[10px] font-black text-white">
            T
          </div>
      ...
      >
        ...
        <div className="p-6 border-b border-neutral-800 flex items-center justify-between">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center shadow-lg shadow-indigo-500/20 font-black">
              T
            </div>
            TPMS{" "}
            <span className="text-indigo-500 text-[10px] font-mono tracking-tighter border border-indigo-500/30 px-1 rounded uppercase">
              Ops
            </span>
          </h2>
          <button
        ...
        <div className="p-4 border-t border-neutral-800 bg-[#0d0d0f]">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 text-zinc-500 hover:text-rose-400 hover:bg-rose-400/5 rounded-xl transition-all group"
          >
  ...
  return (
    <div className="mb-1">
      ...
      >
        ...
        <ChevronDown
          size={14}
          className={`transition-transform duration-300 ${isOpen ? "rotate-180 text-indigo-500" : ""}`}
        />
      </button>
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {items.map((item: any, idx: number) => {
              const isActive =
                currentPath === item.path || (item.isDynamic && currentPath.includes(item.path));
              return (
                <Link
                  key={idx}
                  to={item.path}
                  className={`flex items-center gap-3 px-4 py-2 text-[12px] transition-all relative ${
                    isActive ? "text-indigo-400 font-bold" : "text-zinc-500 hover:text-indigo-400"
                  }`}
                >
                  {isActive && (
                    <motion.div
